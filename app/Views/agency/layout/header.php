...
<body>
 ...
 <nav class="navbar navbar-expand-lg navbar-dark bg-dark px-3 justify-content-between">
  ...
  <div class="d-flex align-items-center gap-3">
   ...
   <div class="dropdown">
    <?php
    $agencyImage = session('profile_img') ?? 'profile-default.png'; // fallback if not set
    ?>
    <a href="#" class="d-flex align-items-center text-white text-decoration-none dropdown-toggle" id="profileDropdown" data-bs-toggle="dropdown" aria-expanded="false">
     <img src="<?= base_url('uploads/employers/' . $agencyImage) ?>" alt="Profile" class="rounded-circle me-2" width="32" height="32">
     <span><?= session('full_name') ?? 'Agency' ?></span>
    </a>
    ...
    <ul class="dropdown-menu <?= $isRTL ? '' : 'dropdown-menu-end' ?> text-small" aria-labelledby="profileDropdown">
     ...
     <li><a class="dropdown-item" href="<?= base_url('agency/edit') ?>"><i class="fas fa-user me-1"></i><?= lang('Dashboard.profile') ?? 'My Profile' ?></a></li>
     <li>
